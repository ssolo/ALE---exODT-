...
int main(int argc, char ** argv)
{
  ...
  // I guess you have to add the openMP bit into sample.cpp while making sure the above number does not change 
  // and also that the tables produed during the calculations work ..
  RandomTools::setSeed(20110426);
  cout << model->sample() << endl;
  // so since we fixed the random seed, the above reconciled tree should alos not change..  
